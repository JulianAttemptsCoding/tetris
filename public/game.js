...
let keys={...defaults,...JSON.parse(localStorage.getItem('tetris-keys')||'{}')};
...
document.querySelectorAll('#bindings input').forEach(input=>input.addEventListener('keydown',e=>{e.preventDefault();if(['Tab','Shift','Control','Alt','Meta'].includes(e.key))return;keys[input.dataset.action]=e.key;saveKeys();input.blur();}));
